...
public class SparseMatrixCSRMul implements MatrixMultiplication {
    ...
    public void multiply(double[][] A, double[][] B, double[][] C, int N) {
        CSRMatrix csrA = convertToCSR(A);
        CSRMatrix csrB = convertToCSR(B);
        ...
        for (int i = 0; i < resultMatrix.rows; i++) {
            for (int j = resultMatrix.rowPointers[i]; j < resultMatrix.rowPointers[i + 1]; j++) {
                denseResult[i][resultMatrix.columnIndices[j]] = resultMatrix.values[j];
            }
        }
